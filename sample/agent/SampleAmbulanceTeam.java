		...
		if (someoneOnBoard()) {
			...
			if (location() instanceof Refuge) {
				...
				if (path != null) {
					...
					if (iAmWorking!=true&&this.isStuck(getMeAsHuman(), true)
							&&this.location()==this.lastPosition &&super.sended!=true)// this.location().equals(this.positionHistory))
							{
								super.sended=true;
	...
	/*	if (this.isStuck(getMeAsHuman(), true)
		...
		if (!targets.isEmpty()) {
			...
			if (iAmWorking!=true&&this.isStuck(getMeAsHuman(), true)
					...
					{
						super.sended=true;
						System.out.print(this.getMeAsHuman().getID()
								+ "AT-> 我在发送被堵信息  位置 :" + this.getID() + "\n");
						// System.out.print("AT -> [SEND]i'M sTUCK~!!! 发送被堵信息\n");
						AgentIsStuckMessage message = new AgentIsStuckMessage(this
								.location().getID());
						this.sendMessage(message, MessagePriority.Medium);
